...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        switch ($_POST['action']) {
            case 'create':
                $stmt = $pdo->prepare("INSERT INTO usuarios (nombre, apellido, fecha_nacimiento, genero, genero_personalizado, contacto, contrasena, rol_user) VALUES (?, ?, ?, ?, ?, ?, ?, 'departamentos')");
                // Para departamentos usamos contraseña simple (no hasheada) como en el sistema original
                $stmt->execute([
                    $_POST['nombre'],
                    $_POST['apellido'], 
                    ...
                    $_POST['contacto'],
                    $_POST['contrasena'] // Sin hashear para departamentos
                ]);
                $mensaje = 'Usuario de departamento creado exitosamente';
            ...
            case 'update':
                ...
                if (!empty($_POST['contrasena'])) {
                    ...
                    $params[] = $_POST['contrasena']; // Sin hashear para departamentos
                }
                
                if (!empty($updateFields)) {
                    $params[] = $_POST['user_id'];
                    $sql = "UPDATE usuarios SET " . implode(', ', $updateFields) . " WHERE id = ? AND rol_user = 'departamentos'";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute($params);
                    $mensaje = 'Usuario de departamento actualizado exitosamente';
                    $tipo_mensaje = 'success';
                }
                break;
                
            case 'delete':
                $stmt = $pdo->prepare("DELETE FROM usuarios WHERE id = ? AND rol_user = 'departamentos'");
                $stmt->execute([$_POST['user_id']]);
                $mensaje = 'Usuario de departamento eliminado exitosamente';
                $tipo_mensaje = 'success';
                break;
        }
    } catch (Exception $e) {
...
// Obtener lista de usuarios de departamentos
$search = $_GET['search'] ?? '';
$sql = "SELECT * FROM usuarios WHERE rol_user = 'departamentos'";
if ($search) {
    ...
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 p-8 ml-64 overflow-y-auto">
            <div class="welcome-message">
                <h1 class="text-3xl font-bold text-[#6B1024] mb-4">Gestión de Departamentos</h1>
                <p class="text-gray-600">Administrar usuarios del área departamental municipal</p>
            </div>
            ...
            <div class="mt-8 space-y-6">
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-[#6B1024]">Lista de Usuarios Departamentales</h2>
                        <p class="text-gray-600">Total: <?php echo count($departamentos); ?> usuarios de departamentos registrados</p>
                    </div>
                ...
                <!-- Tabla de usuarios departamentales -->
                            ...
                            <?php if (empty($departamentos)): ?>
                                <tr>
                                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                        <?php echo $search ? 'No se encontraron usuarios con ese criterio de búsqueda' : 'No hay usuarios de departamentos registrados'; ?>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($departamentos as $departamento): ?>
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 font-medium"><?php echo $departamento['id']; ?></td>
                                        <td class="px-6 py-4">
                                            <?php echo htmlspecialchars($departamento['nombre'] . ' ' . $departamento['apellido']); ?>
                                        </td>
                                        <td class="px-6 py-4"><?php echo htmlspecialchars($departamento['contacto']); ?></td>
                                        <td class="px-6 py-4"><?php echo $departamento['fecha_nacimiento']; ?></td>
                                        <td class="px-6 py-4">
                                            <?php 
                                            echo htmlspecialchars(
                                                $departamento['genero'] === 'otro' && $departamento['genero_personalizado'] 
                                                ? $departamento['genero_personalizado'] 
                                                : ucfirst($departamento['genero'])
                                            ); 
                                            ?>
                                        </td>
                                        <td class="px-6 py-4"><?php echo date('d/m/Y', strtotime($departamento['fecha_registro'])); ?></td>
                                        <td class="px-6 py-4 text-center">
                                            <button onclick="showEditForm(<?php echo htmlspecialchars(json_encode($departamento)); ?>)" 
                                                    class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors mr-2">
                                                Editar
                                            </button>
                                            <button onclick="showDeleteForm(<?php echo $departamento['id']; ?>, '<?php echo htmlspecialchars($departamento['nombre'] . ' ' . $departamento['apellido']); ?>')" 
    ...
    <script>
        function showCreateForm() {
            ...
            modalContent.innerHTML = `
                <h2 class="text-2xl font-bold text-[#6B1024] mb-6">Agregar Usuario Departamental</h2>
        ...
        function showEditForm(departamento) {
            const modal = document.getElementById('modal');
            const modalContent = document.getElementById('modalContent');
            
            modalContent.innerHTML = `
                <h2 class="text-2xl font-bold text-[#6B1024] mb-6">Editar Usuario Departamental</h2>
                <form method="POST" class="space-y-4">
                    <input type="hidden" name="action" value="update">
                    <input type="hidden" name="user_id" value="${departamento.id}">
                    
                    <div class="grid grid-cols-2 gap-4">
                        <input type="text" name="nombre" value="${departamento.nombre}" placeholder="Nombre"
                               class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6B1024] focus:border-transparent">
                        <input type="text" name="apellido" value="${departamento.apellido}" placeholder="Apellido"
                               class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6B1024] focus:border-transparent">
                    </div>
                    
                    <input type="text" name="contacto" value="${departamento.contacto}" placeholder="Email o teléfono"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6B1024] focus:border-transparent">
        ...
        function showDeleteForm(id, nombre) {
            ...
            modalContent.innerHTML = `
                <h2 class="text-2xl font-bold text-red-600 mb-6">Eliminar Usuario Departamental</h2>
                <form method="POST" class="space-y-4">
